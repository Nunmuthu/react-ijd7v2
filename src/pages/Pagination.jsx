...
export class Pagination extends React.Component {
  generatedButton(button) {
    return <button onClick={(button) => this.view(button)}>{button}</button>;
  }
  displaygeneratebutton() {
    let allbuttons = ['pre', 1, 2, 3, 4, 5, , 'next'];
    return allbuttons.map((a) => this.generatedButton(a));
  }
  ...
  // this is method 1
  // displayHead() {
  //   let heading = Object.keys(myData[0]);
  //   let displayHead = heading.map((eachhead) => this.generatehead(eachhead));
  //   return displayHead;
  // }

  // displayrow(data) {
  //   let heading = Object.keys(data);
  //   let diplayEachRow = heading.map((eachKey) =>
  //     this.generateData(data[eachKey])
  //   );

  //   return diplayEachRow;
  // }
  // displaybody(data) {
  //   let displayEachBody = myData
  //     .map((eachRow) => this.generatedata(eachRow))
  //     .join('');
  //   return displayEachBody;
  // }

  // this is method 2
  displaygenerateheading() {
    let heading = Object.keys(myData[0]);
    let displayHead = heading.map((eachhead) => this.generatehead(eachhead));
    return displayHead;
  }

  displaygeneratedata(data) {
    let data1 = myData.map((eachRow) => this.displaygeneraterow(eachRow));
    return data1;
  }

  displaygeneraterow(data) {
    let heading = Object.keys(data);
    let diplayEachRow = heading.map((eachKey) =>
      this.generatedata(data[eachKey])
    );
  ...
  render() {
    return (
      <div>
        <div htmlFor="pagination"></div>
        <div>{this.displaygeneratebutton()}</div>
        <table>
          {/* <thead>{this.displayHead()}</thead>
          <tbody>{this.displaybody()}</tbody> */}
          <thead>{this.displaygenerateheading()}</thead>
          <tbody>{this.displaygeneratedata()}</tbody>
        </table>
      </div>
    );
  }
